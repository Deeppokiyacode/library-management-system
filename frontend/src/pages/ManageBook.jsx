...
const ManageBook = () => {
    ...
    const handleUpdateSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            const res = await axios.put(`http://127.0.0.1:8000/api/books/${editBookId}/update/`, formData, {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this book?")) {
            try {
                const res = await axios.delete(`http://127.0.0.1:8000/api/books/${id}/delete/`);
                if (res.data.success) {
                    toast.success(res.data.message);
                    fetchInitialData();
                            ...
                            <div className="card-body px-4 pb-4">
                                <form onSubmit={handleUpdateSubmit}>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        {currentCoverUrl && (
                                            <div className="mb-2">
                                                <img src={`http://127.0.0.1:8000${currentCoverUrl}`} alt="Current Cover" className="img-thumbnail" style={{ height: "60px", width:"50px", objectFit:"cover" }} />
                                            </div>
                                        )}
                                        <input 
                                            type="file" 
                                                    ...
                                                    <tr key={item.id} className="border-bottom">
                                                        ...
                                                        <td className="py-3 text-start">
                                                            <div className="d-flex flex-column align-items-start">
                                                                {item.cover_image ? (
                                                                     <img src={`http://127.0.0.1:8000${item.cover_image}`} alt="cover" className="rounded mb-1 shadow-sm" style={{width: "45px", height: "55px", objectFit: "cover"}} />
                                                                ) : (
